...
import { Loader2, ShieldCheck, CheckCircle, ScanLine, Hexagon, Zap, AlertTriangle } from 'lucide-react';
...
const LandingPage: React.FC<Props> = ({ onLogin }) => {
  ...
  const [stats, setStats] = useState({ verifiedCredentials: "...", issuers: "...", transactions: "..." });
  ...
  // --------------------------------------------------------------------------
  if (wallet.address) {
    const roles: { role: Role; label: string; title: string; icon: string; description: string; etchedLine?: React.CSSProperties }[] = [
      {
        role: 'student',
        label: 'User Module 01',
        title: 'STUDENT',
        icon: 'school',
        description: 'Manage your sovereign identity and academic achievements in a secure digital vault.',
      },
      {
        role: 'issuer',
        ...
        title: 'ISSUER',
        icon: 'apartment',
        description: 'Mint cryptographically signed credentials and distribute them to verified participants.',
        etchedLine: { background: 'linear-gradient(90deg, transparent, #d946ef, transparent)', top: '80%' },
      },
      {
        role: 'verifier',
        ...
        title: 'VERIFIER',
        icon: 'verified_user',
        description: 'Validate zero-knowledge proofs and verify authenticity without compromising privacy.',
      },
      {
        role: 'governance',
        ...
        title: 'GOVERNANCE',
        icon: 'gavel',
        description: 'Oversee system architecture, adjust protocols, and manage global identity parameters.',
        etchedLine: { background: 'linear-gradient(90deg, transparent, #d946ef, transparent)', top: '15%' },
      },
    ];
    ...
    return (
      <div
        ...
        style={{
          ...
          backgroundImage: 'radial-gradient(circle at 20% 30%, rgba(139, 0, 255, 0.15) 0%, transparent 40%), radial-gradient(circle at 80% 70%, rgba(0, 255, 255, 0.1) 0%, transparent 40%)',
        }}
      >
        ...
        <main className="grid grid-rows-[auto_1fr] min-h-screen p-5 md:p-10 gap-5 md:gap-10 max-w-[1600px] mx-auto relative z-10">
          ...
          <header className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-white/10 pb-5 gap-3">
            <div className="flex flex-col gap-1">
              <div className="glitch-header text-[0.8rem] uppercase tracking-[0.4em] text-neon font-mono" data-text="IDENTITY UNLOCKED">
                IDENTITY UNLOCKED
              </div>
              ...
              {wallet.isMock && (
                <span className="text-[10px] text-amber-500 border border-amber-500/30 px-2 py-0.5 bg-amber-500/10 w-fit">[MOCK]</span>
              )}
              {!wallet.isCorrectNetwork && !wallet.isMock && (
                <div className="flex items-center gap-3 mt-2 bg-amber-500/10 border border-amber-500/30 px-3 py-1.5">
                  ...
                  <button onClick={() => wallet.switchNetwork()} className="text-[10px] font-bold text-amber-500 underline hover:text-amber-400">SWITCH_TO_AMOY</button>
                </div>
              )}
            </div>
              ...
              >
                ...
                <div>
                  <div className="flex items-center gap-4 mb-8 md:mb-10">
                    ...
                    <div className="font-mono text-[0.7rem] tracking-[3px] text-white/50 uppercase">{item.label}</div>
                  </div>
                  <h2 className="text-2xl md:text-[2.2rem] font-black uppercase -tracking-wide text-white leading-tight">{item.title}</h2>
                  <p className="text-sm md:text-[1.1rem] text-white/70 mt-3 mb-8 md:mb-10 font-light max-w-[85%] md:max-w-[80%] leading-relaxed">{item.description}</p>
                </div>

                <button className="init-btn-landing">
                  INITIALIZE <span className="material-symbols-outlined init-arrow">arrow_forward</span>
                </button>
              </div>
            ))}
          </section>
             ...
             <div className="relative w-full max-w-2xl obsidian-panel border-magenta/30 overflow-hidden shadow-[0_0_50px_rgba(217,70,239,0.15)]">
              ...
              <div className="flex items-center justify-between p-6 border-b border-white/10 bg-white/5">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-xl font-bold font-display text-white tracking-widest uppercase">Institutional Verification</h3>
                </div>
                {kybStep === 'form' && (
                  <button onClick={() => setShowKybModal(false)} className="text-text-muted hover:text-white transition-colors">
              ...
              <div className="p-8">
                  ...
                  <div className="space-y-6 animate-in slide-in-from-right-4">
                    <div className="bg-magenta/10 border border-magenta/20 p-4 mb-6 flex gap-3">
                      ...
                      <p className="text-sm text-magenta font-mono leading-relaxed">
                        Fraud prevention protocols active. Institutional validation required for write access to the ledger.
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      ...
                      <div className="space-y-2">
                        <label className="text-xs font-mono text-text-muted uppercase">Registration ID</label>
                        <input
                          type="text"
                          className="w-full bg-black/50 border border-white/10 p-3 text-white font-mono text-sm focus:border-magenta focus:ring-0"
                      ...
                      <div className="space-y-2">
                         <label className="text-xs font-mono text-text-muted uppercase">License Upload</label>
                        <div className="relative">
                          <input
                            type="file"
                            className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                            onChange={(e) => e.target.files && setKybData({...kybData, document: e.target.files[0]})}
                          />
                          <div className={`w-full bg-black/50 border border-dashed ${kybData.document ? 'border-magenta text-magenta' : 'border-white/20 text-text-muted'} p-2.5 text-xs font-mono flex items-center justify-center gap-2 hover:bg-white/5 transition-colors`}>
                             {kybData.document ? (
                ...
                {kybStep === 'approved' && (
                   <div className="py-8 flex flex-col items-center justify-center text-center space-y-6 animate-in zoom-in duration-300">
                      ...
                      <div>
                         <h2 className="text-3xl font-bold font-display text-white uppercase tracking-widest">Verified</h2>
                         <p className="text-text-muted font-mono mt-2 text-sm">Identity Anchor Established.</p>
                      </div>
    ...
    <div className="relative font-body selection:bg-magenta/30 selection:text-white min-h-screen flex flex-col overflow-hidden bg-[#020203]">
      ...
      <nav className="relative z-50 flex items-center justify-end px-6 lg:px-8 py-6 max-w-7xl mx-auto w-full">
          <div className="hidden md:flex items-center gap-8 font-mono text-xs tracking-widest text-slate-400">
              <span className="hover:text-magenta transition-colors cursor-pointer hover:shadow-[0_0_10px_rgba(217,70,239,0.5)]">ARCHITECTURE</span>
              <span className="hover:text-neon transition-colors cursor-pointer hover:shadow-[0_0_10px_rgba(6,182,212,0.5)]">NODES</span>
              ...
              <button
                  ...
                  className="bg-white/5 border border-white/10 px-6 py-2 notch-br hover:border-magenta/50 hover:bg-magenta/10 transition-all text-white active:scale-95 flex items-center gap-2 backdrop-blur-md"
              >
                  {wallet.isConnecting ? <Loader2 className="w-4 h-4 animate-spin" /> : "EXECUTE_NODE"}
              </button>
          </div>
      </nav>
          ...
          <div className="grid lg:grid-cols-12 gap-12 items-center">
              ...
              <div className="lg:col-span-6 space-y-8">
                  ...
                  <h1 className="font-display text-6xl lg:text-[100px] leading-[0.85] font-bold tracking-tighter text-white animate-in slide-in-from-bottom-4 duration-700 delay-100 relative">
                      <span className="block glitch-intense" data-text="TRUSTLESS">TRUSTLESS</span>
                      <span className="text-transparent bg-clip-text bg-gradient-to-r from-magenta via-purple-500 to-neon bg-[length:200%_100%] animate-refract block">VERACITY</span>
                  </h1>
                  ...
                  <p className="text-lg text-slate-400 max-w-xl font-light leading-relaxed animate-in slide-in-from-bottom-4 duration-700 delay-200 border-l-2 border-magenta/50 pl-6">
                      Deploy sovereign academic credentials on the <span className="text-white font-medium">Morningstar Protocol</span>.
                      Immutable. Verifiable. Decentralized.
                  </p>
                  ...
                  <div className="flex flex-wrap gap-6 pt-8 animate-in slide-in-from-bottom-4 duration-700 delay-300">
                      ...
                      <button className="h-14 px-8 flex items-center justify-center obsidian-panel notch-br border-white/10 hover:border-neon/50 hover:bg-neon/10 transition-all group">
                          <span className="font-mono text-xs tracking-widest text-slate-400 group-hover:text-neon transition-colors">DOCUMENTATION</span>
                      </button>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-8 pt-16 border-t border-white/5 max-w-lg animate-in fade-in duration-1000 delay-500">
                      <div>
                          <div className="text-3xl font-bold font-display tracking-tight text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]">{stats.transactions.includes('M') ? stats.transactions : '2.4M'}</div>
                          <div className="text-[10px] font-mono text-magenta tracking-widest uppercase mt-1">TRANSACTIONS</div>
                      </div>
                      <div>
                          <div className="text-3xl font-bold font-display tracking-tight text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]">{stats.issuers}</div>
                          <div className="text-[10px] font-mono text-neon tracking-widest uppercase mt-1">NODES</div>
                      </div>
                      <div>
                          <div className="text-3xl font-bold font-display tracking-tight text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]">12<span className="text-slate-400 text-sm">ms</span></div>
                          <div className="text-[10px] font-mono text-accent tracking-widest uppercase mt-1">LATENCY</div>
                      </div>
                  </div>
              </div>
              ...
              <div className="lg:col-span-6 relative h-[600px] flex items-center justify-center animate-in fade-in duration-1000">
                  ...
                  <svg className="absolute inset-0 w-full h-full pointer-events-none z-0" viewBox="0 0 600 600">
                      ...
                      >
                         <animate attributeName="stroke-dashoffset" from="210" to="0" dur="3s" repeatCount="indefinite" />
                      </path>
                      ...
                      <path
                        ...
                        filter="url(#glow)"
                      >
                         <animate attributeName="stroke-dashoffset" from="210" to="0" dur="2s" repeatCount="indefinite" />
                      </path>
                      ...
                      <path
                        ...
                        filter="url(#glow)"
                      >
                         <animate attributeName="stroke-dashoffset" from="210" to="0" dur="4s" repeatCount="indefinite" />
                      </path>
                  </svg>
                  ...
                  <div className="absolute top-[10%] left-1/2 -translate-x-1/2 z-20 hover:scale-105 transition-transform duration-500">
                      <div className="w-64 glass-panel rounded-xl p-6 text-center border-t border-magenta/50 shadow-[0_0_30px_-10px_rgba(217,70,239,0.3)] relative overflow-hidden group">
                          ...
                          <h3 className="text-white font-bold font-display tracking-widest text-lg">ISSUER</h3>
