...
const Layout: React.FC<Props> = ({ role, onLogout, children }) => {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden flex flex-col screen-tear">
      ...
      {/* Ambient Glows */}
      <div className="fixed top-[-10%] right-[-10%] w-[600px] h-[600px] bg-primary/10 rounded-full blur-[120px] pointer-events-none z-0 animate-float"></div>
      <div className="fixed bottom-[-10%] left-[-10%] w-[500px] h-[500px] bg-highlight/10 rounded-full blur-[100px] pointer-events-none z-0 animate-float" style={{ animationDelay: '4s' }}></div>
        ...
        <nav className="border-b border-white/5 sticky top-0 z-40 backdrop-blur-xl bg-background-dark/80">
          <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <span className="font-bold text-white tracking-tight text-xl font-display block leading-none">
                    <GlitchText text="MORNINGSTAR" intensity="intense" />
                </span>
                <span className="text-[10px] font-mono text-data tracking-widest uppercase">Protocol v3.0.1</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-6">
               ...
               <div className="flex items-center gap-4">
                 <div className="flex items-center gap-2 text-sm text-text-muted bg-white/5 px-4 py-2 rounded-none border-l-2 border-primary hover:bg-white/10 transition-colors cursor-pointer cyber-corner">
                   <span className="material-symbols-outlined text-primary text-sm">fingerprint</span>
                   <span className="hidden md:inline font-mono">{wallet.displayAddress || '0x...'}</span>
